...
p = {'Ec':10*1e3,'Etac':100e3,'hc': 1e-6, 'vc': 0.5,'Es':20e3, 'hs': 50e-6, 'vs': 0.5,'Ya':1e9,'AIC':0,'Lc':50e-6}
...
def f(x, *args):
    ...
    np.savetxt("strain_energy_%s.txt"%(ar), np.array([np.array(all_times)-lag_time, all_energies]).T, \
               header="Time\tModel")
    np.savetxt("updated_params_baseline_%s.txt" % (ar), x)
