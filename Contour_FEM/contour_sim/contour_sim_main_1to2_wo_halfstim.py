...
def doublet_simulation(fb,sx_increase,sy_increase,ea_input,l_act_input):
    ...
    u_xdx_ani_array = np.empty_like(np.linspace(0, d, 200))
    u_ydx_ani_array = np.empty_like(np.linspace(0, d, 200))
    ...
    ea_array = np.empty_like(np.linspace(0, d, 200))
    for v in vertices(mesh):
    ...
    activation_profile_sigma_x = Expression("(amp-fb)*1/(1+exp(0.65*(x[0]-15.7)))+fb",degree=0,amp=sx_increase,fb=feedback_x)  # Expression("1/(1+exp(0.6*(x[0]-11.5)))/10 ",degree=0)#Expression("1/(1+exp(0.6*(x[0]-11.5)))/10 ",degree=0)
    activation_profile_sigma_y = Expression("(amp-fb)*1/(1+exp(0.65*(x[0]-15.7)))+fb",degree=0,amp=sy_increase,fb=feedback_y) # Expression("1/(1+exp(0.6*(x[0]-11.5)))/5 ",degree=0)#Expression("(0.1-0.09)*1/(1+exp(0.8*(x[0]-11.5)))+0.09",degree=0)#Expression("1/(1+exp(0.6*(x[0]-11.5)))/10 ",degree=0)#Constant(0.12)
    ...
    u_xdx_array = np.empty_like(np.linspace(0, d, 200))
    u_ydx_array = np.empty_like(np.linspace(0, d, 200))
    for v in vertices(mesh):
    ...
    activation_profile_landa_array = np.empty_like(np.linspace(0, d, 200))
    activation_profile_sigma_x_array = np.empty_like(np.linspace(0, d, 200))
    activation_profile_sigma_y_array = np.empty_like(np.linspace(0, d, 200))
    for v in vertices(mesh):
    ...
    region = AutoSubDomain(lambda x, on: x[0] <= d/2)
    region.mark(cf, 1)
    ...
    def interpolate_contour(u_x_array, u_y_array, u_x_array_ani, u_y_array_ani):
        ...
        l_0 = np.absolute(f_init(x_range) + d/2)
        l = np.absolute(f(x_range) + d/2)
    ...
    y_halfstim = np.loadtxt("contour_1to2.txt", skiprows=1)
...
for ea in EA_array:
    for fb in Feedback_array:
        l_act_input = 77.38 #nN
        sx_increase_input = 0.114
        sy_increase_input = 0.243
...
with open('all_results/halfstim_1to2_simulation.dat',"wb") as f:
    pickle.dump(dict_allEA, f,protocol=pickle.HIGHEST_PROTOCOL)

with open('all_results/halfstim_1to2_simulation.pickle',"wb") as f:
    pickle.dump(dict_allEA, f,protocol=pickle.HIGHEST_PROTOCOL)
