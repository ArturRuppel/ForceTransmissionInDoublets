...
def plot_value_over_time(x, y, ylim, xlabel, ylabel, title):
    ...
    se_y = std_y/np.sqrt(np.shape(y)[1])
...
def plot_twovalues_over_time(x, y1, y2, ylim, xlabel, ylabel,titleleft, titleright, titleboth):
    ...
    se_y1 = std_y1/np.sqrt(np.shape(y1)[1])
    mean_y2 = np.nanmean(y2,axis=1)
    std_y2 = np.nanstd(y2,axis=1)
    se_y2 = std_y2/np.sqrt(np.shape(y2)[1])
...
#%% Plot figure 3E
x = np.arange(0,60)
y1 = AR1to1d_fullstim_long["MSM_data"]["relsigma_yy_average"]
y2 = AR1to1s_fullstim_long["MSM_data"]["relsigma_yy_average"]
ylim = (0.9,1.2)
xlabel = "time [min]"
ylabel = "sigma_{yy, rel}"
titleleft = "doublet"
titleright = "singlet"
titleboth = "Relative yy-stress full stim"
plot_twovalues_over_time(x, y1, y2, ylim, xlabel, ylabel,titleleft, titleright, titleboth)
...
# save figure
fig.savefig(folder+'fig3A.png', dpi=300, bbox_inches="tight")
# plt.close()
#%% plot figure 3X
...
sigmaxx_2to1d_diff_crop = sigmaxx_2to1d_diff[crop_start:crop_end,crop_start:crop_end]*1e3
...
axes[1,0].imshow(sigmaxx_2to1d_diff_crop, cmap=plt.get_cmap("seismic"), interpolation="bilinear", extent=extent, vmin=sigma_min, vmax=sigma_max, aspect='auto')

axes[1,1].imshow(sigmayy_2to1d_diff_crop, cmap=plt.get_cmap("seismic"), interpolation="bilinear", extent=extent, vmin=sigma_min, vmax=sigma_max, aspect='auto')
...
# save figure
fig.savefig(folder+'fig3B.png', dpi=300, bbox_inches="tight")
# plt.close()