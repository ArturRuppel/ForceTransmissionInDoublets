...
iter=3;%after each interation the window size is divided by 2
overlapPIV=128;%in pixels (applied to first window)                  %32
overlapTrack=64;%in pixels (applied to last window befor tracking)  %16
interval=8;
pas=interval*pix;
%Tracking parameters 
...
masscut=250000; %250000
maxd=3;%deplacement max d'une bille entre les 2 images
r_neighbor = 50; % in pixel
corr_th = 0.5;


% pix = 0.065e-6;% taille du pixel %60x 0.091 - 40x 0.133 
% E=1700; % elasticité du substrat (Pa)
% nu=0.5; %Poisson ratio
% alphadef=1e-19;   %*(100000/E)^2; %this coef of regularization should vary with the young modulus _TOCHECK!!!
% %initial window size
% window=128;
...
% pas=interval*pix;
% %Tracking parameters 
% featsize=5;% rayon de la particule en pixel, detect?e par un masque de taille featsize typiquement 2 ou 3
% barrg=12; % rayon de giration (reli? ? la taille de la particule en pixel)
% barcc=0.2; % excentricit? (0 = rond 1 = pas rod du tout) typiquement entre 0.1 OU 0.2
% IdivRg=0;%intensit? int?gr?e minimale pour detecter une particule
% masscut=220000; %250000
...
Vect2cells=zeros(1,Nb_frames);% somme vectorielle des tractions cellules-subtrat: a l'équilibre, devrait etre egale a zero (systeme isole)
...
Angle=zeros(1,Nb_frames);% angle d'orientation principale par rapport à la verticale
...
%     r_corr = mean(deplaceX(i)*deplaceX(index_neighbors)+deplaceY(i)*deplaceY(index_neighbors)./(sqrt(deplaceX(i)^2+deplaceY(i)^2)*sqrt(deplaceX(index_neighbors).^2+deplaceY(index_neighbors).^2)));
%     if r_corr < corr_th
...
U=pas^2/2*sum(u(:));
disp(['Energie contractile: ',num2str(U), ' J']);
Ec2cells(ff)=U;
...
disp(['contrainte moyenne: ',  num2str(TmagnCROPMOY),' Pa']);
...
%imageinv=255-double(p_corr);
%subplot(2,2,3),colormap(gray)
% imshow(double(p_corr)+40*double(m1_corr)+60*double(m2_corr),[],'InitialMagnification','fit')
% hold on
% quiver(gridX(1:2:end,1:2:end),gridY(1:2:end,1:2:end),Tractionx(1:2:end,1:2:end), Tractiony(1:2:end,1:2:end),2,'r')
...
subplot(2,3,1),plot(pos,MOY2cells,'o-'),title('Contrainte moyenne (Pa)');
subplot(2,3,2),plot(pos,Total2cells,'s-r'),title('Somme des tractions en norme (N)');
subplot(2,3,3),plot(pos,Vect2cells./Total2cells*100,'d-g'),title('Pourcentage hors equilibre');
subplot(2,3,4), plot(pos,Ec2cells, '--o'),title('Energie contractile (J)');
subplot(2,3,5), plot(pos,Fcellcell,'-s'), hold on, plot(pos,DFcellcell,'-sr'), title('F cell-cell(bleu) et incertitude(rouge) en N')
subplot(2,3,6), plot(pos,Polar,'-sc'),  title('Degre de polarisation')
...
subplot(2,3,1), plot(pos,Momentcell1,'-og'), hold on, plot(pos,Momentcell2,'-om'),hold off,title('Moment contractile net en N.m (cell 1=vert - cell 2=magenta)')
subplot(2,3,2), plot(pos,MOYcell1,'-+g'), hold on, plot(pos,MOYcell2,'-+m'),hold off, title('Contrainte moyenne (cell 1=vert - cell 2=magenta) en Pa')
subplot(2,3,3), plot(pos,Totalcell1,'-og'), hold on, plot(pos,Totalcell2,'-om'), hold off,title('Somme tractions (cell 1=vert - cell 2=magenta) en N')
subplot(2,3,4), plot(pos,Cell_ECMratio1*100,'-dg'), hold on, plot(pos,Cell_ECMratio2*100,'-dm'),hold off, title('Rapport Fcell-cell/Fcell-ECM en %')
subplot(2,3,5), plot(pos,Tmaxcell1,'-+g'), hold on, plot(pos,Tmaxcell2,'-+m'),hold off, title('Contrainte max (cell 1=vert - cell 2=magenta) en Pa')
subplot(2,3,6), plot(pos,Forcex_cell1,'-og'), hold on, plot(pos,Forcey_cell1,'-dg'),hold on, plot(pos,Forcex_cell2,'-om'), hold on, plot(pos,Forcey_cell2,'-dm'),hold off,...
    legend('Fx cell1','Fy cell1','Fx cell2','Fy cell2'),title('Somme tractions X / tractions Y')
...
% %  y0=s.Centroid(:,2);
% figure(h2)
% imshow(cellule(:,:,i),[],'InitialMagnification','fit')
% axis equal
% set(gca,'Visible','off');
...
% 
% 
% writerObj = VideoWriter(fullfile(path,'forces_amplitude1.avi'));
% writerObj.FrameRate = 8;
% open(writerObj);
% 
