...
AR1to2d_halfstim_CM = filter_data_main(AR1to2d_halfstim, AR1to2d_halfstim_CM, "AR1to2d_halfstim")
AR1to1d_halfstim_CM = filter_data_main(AR1to1d_halfstim, AR1to1d_halfstim_CM, "AR1to2d_halfstim")
AR2to1d_halfstim_CM = filter_data_main(AR2to1d_halfstim, AR2to1d_halfstim_CM, "AR1to2d_halfstim")
...
for key1 in AR1to1d_halfstim_CM:
    ...
    for key2 in AR1to1d_halfstim_CM[key1]:    # keys are the same for all dictionaries so I'm just taking one example here
        if key2 in keys_to_load:  # only 1D data can be stored in the data frame
            ...
            concatenated_data[key2] = np.concatenate((concatenated_data_1to2d[key2], concatenated_data_1to1d[key2], concatenated_data_2to1d[key2]))
...
df.to_csv(folder + 'analysed_data/baseline_data.csv', index=False)
df_fullstim = df
...
list_of_quantities = {'a [um]', 'b [um]', 'line tension [nN]', 'sigma_x_CM [mN/m]', 'sigma_y_CM [mN/m]'}
stats = []
for quantity in list_of_quantities:
    stats.append(calculate_and_print_mean_and_CI(df_fullstim, quantity))
stats = pd.concat(stats)
stats.to_csv(folder + 'analysed_data/stats_baseline.csv')
